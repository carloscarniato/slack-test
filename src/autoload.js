...
const routeParamPattern = /\/_/gi
const routeMixedParamPattern = /__/g

const defaults = {
  scriptPattern: /((^.?|\.[^d]|[^.]d|[^.][^d])\.ts|\.js|\.mjs)$/i,
  indexPattern: /^index(\.ts|\.js|\.mjs)$/i,
  autoHooksPattern: /^[_.]?auto_?hooks(\.ts|\.js|\.mjs)$/i,
...
const fastifyAutoload = async function autoload(fastify, options) {
  ...
  const pluginArray = [].concat.apply(
    [],
    Object.values(pluginTree).map((o) => o.plugins),
  )
  const hookArray = [].concat.apply(
    [],
    Object.values(pluginTree).map((o) => o.hooks),
  )

  await Promise.all(
    pluginArray.map(({ file, type, prefix }) => {
...
function getScriptType(fname, packageType) {
  return modulePattern.test(fname)
    ? 'module'
    : typescriptPattern.test(fname)
    ? 'typescript'
    : packageType
}
...
async function findPlugins(dir, options, hookedAccumulator = {}, prefix, depth = 0, hooks = []) {
  ...
  if (packageDirent && !indexDirent) {
    throw new Error(
      `@fastify/autoload cannot import plugin at '${dir}'. To fix this error rename the main entry file to 'index.js' (or .mjs, .ts).`,
    )
  }
    ...
    if (dirent.isDirectory() && !atMaxDepth) {
      ...
      if (options.autoHooks && options.cascadeHooks) {
        directoryPromises.push(
          findPlugins(file, options, hookedAccumulator, prefixBreadCrumb, depth + 1, currentHooks),
        )
      } else {
        directoryPromises.push(
          findPlugins(file, options, hookedAccumulator, prefixBreadCrumb, depth + 1),
        )
      }
...
async function loadPlugin({ file, type, directoryPrefix, options, log }) {
  ...
  const content = await import(url.pathToFileURL(file).href)
  ...
  const prefixOverride =
    plugin.prefixOverride !== undefined
      ? plugin.prefixOverride
      : content.prefixOverride !== undefined
      ? content.prefixOverride
      : undefined
  const prefix =
    (plugin.autoPrefix !== undefined
      ? plugin.autoPrefix
      : content.autoPrefix !== undefined
      ? content.autoPrefix
